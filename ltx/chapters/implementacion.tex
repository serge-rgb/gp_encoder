...
trabaja con granularidad de grupos de workitems llamados \emph{wavefronts}. El
\emph{work group} es una abstracción que se provee para proveer al
...
Cada \emph{work item} tiene un número limitado de registros y un espacio de
memoria local (a veces llamada stack, por su equivalente operacional en CPUs).
Cada wavefront tiene \emph{memoria compartida}, con acceso de lectura y
escritura para cualquier miembro del grupo. Todos los wavefronts tienen acceso
...
Para múltiples threads, se utilizan threads trabajadores que consumen unidades de trabajo. Como se muestra con el siguiente pseudo código:
...
%TODO comparar perf con / sin truco de branch.

\subsection{CPU: Memoria}\label{sub:cpu-mem}
...
Para el algoritmo paralelo, nos vemos forzados a alojar memoria. La razón es
que mientras TinyJPEG procesa la imagen un bloque a la vez, DummyJPEG recibe
$N$ bloques y escribe $N$ resultados en paralelo. Esto obliga al sistema a
alojar memoria para los bloques y para los arreglos donde se guardan los
resultados.


% TODO: Hacer una nota sobre el manejo de memoria. Arenas y demás
